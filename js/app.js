...
// Gallery Images Data List
const GALLERY_IMAGES = [
  'https://kesariyafarm.com/cdn/shop/files/ghee_1000ml_6.1.png?v=1771940051&width=1445',
  'https://kesariyafarm.com/cdn/shop/files/Front_Spoon_1000_ml.png?v=1771942989&width=1445',
  'https://kesariyafarm.com/cdn/shop/files/Front_100_ml.png?v=1771943053&width=1445',
  'https://kesariyafarm.com/cdn/shop/files/1000ml_Pack_of_2.png?v=1771943053&width=1445',
  'https://kesariyafarm.com/cdn/shop/files/Nutrition_1000_ml.png?v=1771942989&width=1445',
  'https://kesariyafarm.com/cdn/shop/files/Price_Manufacture_1000_ml.png?v=1771942989&width=1445'
];
...
/* -------------------------------------------------------------------------- */
/* 1. MULTI-PLATFORM GALLERY & LIGHTBOX ZOOM ENGINE                           */
/* -------------------------------------------------------------------------- */
function initGallery() {
  const mainImg = document.getElementById('mainProductImg');
  const mainImgContainer = document.getElementById('mainImgContainer');
  const thumbs = document.querySelectorAll('.thumb-item');
  const counterBadge = document.getElementById('imageCounterBadge');
  ...
  const lbPrevArrow = document.getElementById('lbPrevArrow');
  const lbNextArrow = document.getElementById('lbNextArrow');
  const lightboxViewport = document.getElementById('lightboxViewport');

  // Core Gallery Image Switching Function
  function selectGalleryImage(index) {
    if (index < 0) index = GALLERY_IMAGES.length - 1;
    if (index >= GALLERY_IMAGES.length) index = 0;
    
    state.currentImageIndex = index;
    const newSrc = GALLERY_IMAGES[index];

    if (mainImg) mainImg.src = newSrc;
    if (zoomedImg) zoomedImg.src = newSrc;

    // Update Counter Badge
    if (counterBadge) counterBadge.innerText = `${index + 1} / ${GALLERY_IMAGES.length}`;
    if (modalCounter) modalCounter.innerText = `${index + 1} of ${GALLERY_IMAGES.length}`;
  ...
  // Prev / Next Main Gallery Buttons
  if (galleryPrevBtn) {
    galleryPrevBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      selectGalleryImage(state.currentImageIndex - 1);
    });
  }
  ...
  function handleSwipe() {
    ...
    if (Math.abs(diff) > 40) {
      if (diff > 0) {
        selectGalleryImage(state.currentImageIndex + 1); // Swipe Left -> Next
      } else {
        selectGalleryImage(state.currentImageIndex - 1); // Swipe Right -> Prev
      }
    }
  }
  ...
  function openLightbox() {
    if (zoomedImg) zoomedImg.src = GALLERY_IMAGES[state.currentImageIndex];
    if (modalCounter) modalCounter.innerText = `${state.currentImageIndex + 1} of ${GALLERY_IMAGES.length}`;
  ...
  if (closeZoomBtn) closeZoomBtn.addEventListener('click', closeLightbox);

  // Modal Prev / Next Buttons
  if (modalPrevBtn) modalPrevBtn.addEventListener('click', () => selectGalleryImage(state.currentImageIndex - 1));
  if (modalNextBtn) modalNextBtn.addEventListener('click', () => selectGalleryImage(state.currentImageIndex + 1));
  if (lbPrevArrow) lbPrevArrow.addEventListener('click', () => selectGalleryImage(state.currentImageIndex - 1));
  if (lbNextArrow) lbNextArrow.addEventListener('click', () => selectGalleryImage(state.currentImageIndex + 1));

  // Zoom In / Out Engine
  function updateZoomTransform() {
    if (zoomedImg) {
      zoomedImg.style.transform = `scale(${state.zoomScale}) translate(${state.panX}px, ${state.panY}px)`;
    }
    if (modalZoomLevel) {
      modalZoomLevel.innerText = `Zoom: ${Math.round(state.zoomScale * 100)}%`;
    }
  }

  function zoomIn() {
    if (state.zoomScale < 3.0) {
...
function initCheckout() {
  ...
  const quickDemoFillBtn = document.getElementById('quickDemoFillBtn');

  window.openCheckoutModal = () => {
    if (state.cart.length === 0) {
      showToast('Your cart is empty! Please select a size first.', 'warning');
      return;
    }
  ...
  // Quick Fill Demo Address for testing
  if (quickDemoFillBtn) {
    quickDemoFillBtn.addEventListener('click', () => {
      document.getElementById('custName').value = 'Rajesh Kumar';
      document.getElementById('custPhone').value = '9876543210';
      document.getElementById('custAddress').value = '102, Green Park Avenue, Bandra West';
      document.getElementById('custCity').value = 'Mumbai';
      document.getElementById('custPincode').value = '400050';
      document.getElementById('custState').value = 'Maharashtra';

      // Clear any error highlights
      ['custName', 'custPhone', 'custAddress', 'custCity', 'custPincode'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.style.borderColor = 'var(--border-color)';
      });

      showToast('Demo shipping address auto-filled!', 'success');
    });
  }

  if (nextToStep2) {
    nextToStep2.addEventListener('click', validateAndAdvanceStep1);
  }
...
function validateAndAdvanceStep1() {
  ...
  fields.forEach(f => {
    ...
    if (el) {
      if (!el.value.trim()) {
        el.style.borderColor = '#D9381E';
        isValid = false;
      } else {
        el.style.borderColor = 'var(--border-color)';
      }

      // Add input listener to clear red highlight on typing
  ...
  if (!isValid) {
    showToast('Please fill out all required shipping fields! (Or click Quick Fill)', 'warning');
    return false;
  }
...
window.handleCheckoutFormSubmit = function(e) {
  e.preventDefault();
  const step1Content = document.getElementById('step1Content');
  
  if (step1Content && step1Content.style.display !== 'none') {
    validateAndAdvanceStep1();
  } else {
    handlePlaceOrder(e);
  }
};
